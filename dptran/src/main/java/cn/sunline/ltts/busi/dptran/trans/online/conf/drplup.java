...
public class drplup {
	 ...
	 * 
	 * @Title: drpupt
	 * @Description: 修改存款支取计划控制部件信息
	 * @param input
	 * @author zhangjunlei
	 * @date 2016年7月14日 下午2:35:09
	 * @version V2.3.0
	 */
	public static void drpupt(
			final cn.sunline.ltts.busi.dptran.trans.online.conf.intf.Drplup.Input input) {
		...
		// 产品支取计划控制信息不存在,不能修改
		tblDppb_drpl_temp = KupDppbDrplTempDao.selectOne_odb1(prodcd,
				crcycd, false);
		if (CommUtil.isNull(tblDppb_drpl_temp)) {
			throw DpModuleError.DpstComm.BNAS2165();
		}
		...
		if (tblDppb_part_temp.getPartfg() == E_YES___.NO) {
			...
			KupDppbDrplTempDao.updateOne_odb2(tblDppb_drpl_temp);// 修改存款支取计划控制部件信息
		}
	}
