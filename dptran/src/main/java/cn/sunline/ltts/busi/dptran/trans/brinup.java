...
public class brinup {
	 ...
	 * @version V2.3.0
	 */
	public static void updPrdInterst( final cn.sunline.ltts.busi.dptran.trans.online.conf.Brinup.Input input,  final cn.sunline.ltts.busi.dptran.trans.online.conf.Brinup.Output output){
...
//					SysUtil.getInstance(IoIntrSvrType.class).SelRlirByintrcd(brchno, input.getBsincd());
		...
		//违约支取利息类型
		E_TEARTP drintp = input.getTeartp();
		//利息组代码
		String ingpcd = "8888";
				
		//查询产品违约支取利息信息
		KupDppbDfirTemp tblKupDppbDfirTemp = KupDppbDfirTempDao.selectOne_odb1(prodcd, crcycd, drintp, ingpcd, E_INTRTP.ZHENGGLX, false);
		
		if(CommUtil.isNull(tblKupDppbDfirTemp)){
			throw DpModuleError.DpstProd.BNAS1359();
		}
		
		//主键之一不能修改
		if(drintp != tblKupDppbDfirTemp.getTeartp()){
			throw DpModuleError.DpstProd.BNAS1360();
		}
		
		//法人代码
		tblKupDppbDfirTemp.setCorpno(CommTools.getBaseRunEnvs().getBusi_org_id());
		//产品编号
		tblKupDppbDfirTemp.setProdcd(prodcd);
		// 币种
		tblKupDppbDfirTemp.setCrcycd(crcycd);
		//违约支取利息类型
		tblKupDppbDfirTemp.setTeartp(input.getTeartp());
		// 利率代码类型
		tblKupDppbDfirTemp.setIncdtp(input.getIncdtp());
		// 违约利率代码
		tblKupDppbDfirTemp.setBsincd(input.getBsincd());
		//利息类型
		tblKupDppbDfirTemp.setIntrtp(E_INTRTP.ZHENGGLX);
		//违约利率代码
		tblKupDppbDfirTemp.setIngpcd(ingpcd);
		//违约计息基础
		tblKupDppbDfirTemp.setBsinrl(input.getBsinrl());
		//违约利息支付方法说明
		tblKupDppbDfirTemp.setDrintx(input.getDrintx());
		//违约利息调整类型
		tblKupDppbDfirTemp.setInadtp(input.getInadtp());
		//违约利率靠档标志
		tblKupDppbDfirTemp.setInclfg(input.getInclfg());
		//违约利率靠档方式
		tblKupDppbDfirTemp.setIntrwy(input.getIntrwy());
		//违约利率确定方式
		tblKupDppbDfirTemp.setInsrwy(input.getInsrwy());
		//基准结息金额来源
		tblKupDppbDfirTemp.setBsinam(input.getBsinam());
		//违约结息起始日来源
		tblKupDppbDfirTemp.setBsindt(input.getBsindt());
		//违约结息终止日来源
		tblKupDppbDfirTemp.setInedsc(input.getInedsc());
		//违约支取是否扣除已付利息
		tblKupDppbDfirTemp.setDrdein(input.getDrdein());
		// 利率确定日期
		tblKupDppbDfirTemp.setIntrdt(input.getIntrdt());
		// 违约靠档规则
		tblKupDppbDfirTemp.setLevety(input.getLevety());
				
		
		KupDppbDfirTempDao.updateOne_odb1(tblKupDppbDfirTemp);
	}
