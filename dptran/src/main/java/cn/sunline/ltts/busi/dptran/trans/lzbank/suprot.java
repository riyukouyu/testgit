...
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Suprot.Input;
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Suprot.Output;
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Suprot.Property;
...
public static void dealSuprot( final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Suprot.Input input,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Suprot.Property property,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Suprot.Output output){
        ...
        String servtp=input.getServtp();//交易渠道
        ...
        if(isflag==E_YES___.NO){
             ...
             * 3,账户信息校验
             */
            KnaAcdc tblKnaAcdc =KnaAcdcDao.selectOne_odb2(input.getOtcard(), false); //获取卡和电子账号映射 
            ...
            chkIN.setOpcard(input.getIncard());//对方账户卡号 -转入方账号
            chkIN.setOppona(input.getInacna());//对方户名-转入方户名
            ...
            cn.sunline.ltts.busi.iobus.servicetype.dp.IoCaSevAccountLimit.IoAcSubQuota.InputSetter qtIn = SysUtil
                    .getInstance(cn.sunline.ltts.busi.iobus.servicetype.dp.IoCaSevAccountLimit.IoAcSubQuota.InputSetter.class);
            cn.sunline.ltts.busi.iobus.servicetype.dp.IoCaSevAccountLimit.IoAcSubQuota.Output qtOt = SysUtil
                    .getInstance(cn.sunline.ltts.busi.iobus.servicetype.dp.IoCaSevAccountLimit.IoAcSubQuota.Output.class);
            ...
            drawDpAcctIn.setOpacna(input.getInacna());
            drawDpAcctIn.setToacct(input.getIncard());
            drawDpAcctIn.setCrcycd(tblKnaAcct.getCrcycd());
            drawDpAcctIn.setBankcd(input.getBrchno());
...
//          iobl.setCuacno();//转入方电子账号
            iobl.setIoflag(E_IOFLAG.OUT);//出入金标志
            iobl.setKeepdt(input.getKeepdt());//清算时间
            iobl.setCapitp(capitp);

            iobl.setToacno(tblKnaAcct.getAcctno());
            iobl.setToscac(tblKnaAcdc.getCustac());
            iobl.setBusino(busino);
//          iobl.setAcctno(); //转入方子账号
...
//          iobl.setTobrch(input.getTobrch()); //对方账号所属机构
            ...
            iobl.setToacct(input.getOtcard()); //转出方账号
...
//          iobl.setIsspan(input.getIsspan()); //是否跨法人
            ...
            GlKnaAcct glKnaAcct = GlKnaAcctDao.selectOne_odb1(input.getOtcard(), false);
...
//            }
            ...
            acdrIn2.setToacct(input.getIncard());
...
//          iobl.setIsspan(input.getIsspan()); //是否跨法人
            ...
            //交易明细冲正事件
            IoApRegBook cplinput = SysUtil.getInstance(IoApRegBook.class);
            cplinput.setCustac(input.getOtcard());
            cplinput.setEvent1(input.getFronsq());
            cplinput.setEvent2(input.getFrondt());
            cplinput.setTranev(tranev);
	           // ApStrike.regBook(cplinput);
            ...
            //清算明细冲正注册
            IoApRegBook cplinput2 = SysUtil.getInstance(IoApRegBook.class);
            cplinput2.setCustac(input.getOtcard());
            cplinput2.setEvent1(CommTools.getBaseRunEnvs().getMain_trxn_seq());
            cplinput2.setEvent2(CommTools.getBaseRunEnvs().getTrxn_date());
            cplinput2.setTranev(tranev2);
	           // ApStrike.regBook(cplinput);
...
private static void chkParam(Input input) {
    ...
    if (input.getIotype() == E_IOTYPE.OUT) {
