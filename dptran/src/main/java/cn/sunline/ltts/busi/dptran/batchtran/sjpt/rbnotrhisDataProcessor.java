...
import cn.sunline.ltts.busi.dp.type.sjpt.sjptdt.rbnotrhisInfo;
...
public class rbnotrhisDataProcessor extends
  BatchDataProcessorWithoutDataItem<cn.sunline.ltts.busi.dptran.batchtran.sjpt.intf.Rbnotrhis.Input, cn.sunline.ltts.busi.dptran.batchtran.sjpt.intf.Rbnotrhis.Property> {
	private static final BizLog bizlog = BizLogUtil.getBizLog(rbnotrhisDataProcessor.class);
	/**
	 * 批次数据项处理逻辑。
	 * 
	 ...
	 */
	 @Override
	public void process(cn.sunline.ltts.busi.dptran.batchtran.sjpt.intf.Rbnotrhis.Input input, cn.sunline.ltts.busi.dptran.batchtran.sjpt.intf.Rbnotrhis.Property property) {
		// 获取非金融交易明细明细文件相关数据
		KnpParameter tblKnpParameter = KnpParameterDao.selectOne_odb1("Sjpt.rbnotrhis","rbnotrhisFile", "rbnotrhisData", "%", true);
...
*/		// 产生文件的日期目录
		...
		try {
			...
			DaoUtil.selectList(sjptdtDao.namedsql_selRbnotrhis, params, new CursorHandler<rbnotrhisInfo>() {
						@Override
						public boolean handle(int arg0, rbnotrhisInfo arg1) {
							...
							if (CommUtil.isNotNull(trandt)) {
								trandt = trandt.substring(0, trandt.indexOf("."));
							}
							lnreq.append(trandt).append(E_OFCHAR.VER);
							//交易类型
							String trntye =arg1.getTrntye();
							lnreq.append(trntye).append(E_OFCHAR.VER);
							//客户名称
							String custna = arg1.getTrntye();
							lnreq.append(custna).append(E_OFCHAR.VER);
							//客户号
							String custno = arg1.getCustno();
							...
							//交易状态 默认成功 1 TODO
							lnreq.append("1").append(E_OFCHAR.VER);
							//操作柜员
