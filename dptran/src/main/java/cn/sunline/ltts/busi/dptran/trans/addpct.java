...
import cn.sunline.ltts.busi.dptran.trans.intf.Addpct.Input.DepttmInfo;
...
public class addpct {
	...
	private static final BizLog bizlog = BizLogUtil.getBizLog(addpct.class);

public static void addKupDppdCust( final cn.sunline.ltts.busi.dptran.trans.intf.Addpct.Input input){
		bizlog.debug("交易开始");
	...
	/*	// 当属性为 指定到期日时，存期只能为 空
		...
		for (DepttmInfo tmInfo : input.getDepttmInfo()) {
			...
			if (CommUtil.isNull(tmInfo.getDepttm())) {
				...
				// 浙农信不用存期
			if (E_TERMCD.ALL == tmInfo.getDepttm() || E_TERMCD.T905 == tmInfo.getDepttm() || E_TERMCD.T906 == tmInfo.getDepttm()
					|| E_TERMCD.T907 == tmInfo.getDepttm() || E_TERMCD.T908 == tmInfo.getDepttm() || E_TERMCD.T909 == tmInfo.getDepttm()){
				throw DpModuleError.DpstProd.BNAS1346();
			}

				// 当属性为 无到期日时，存期只能为 活期
//				if (madtby == E_MADTBY.NO) {
//
//					if (tmInfo.getDepttm() != E_TERMCD.T000) {
//						throw DpModuleError.DpstComm
//								.E9999("存期当属性为 无到期日时，存期只能为 活期输入不能为空");
//					}
//				}

				// 当属性为根据开户日计算到期日或者根据首次存入日计算到期日时，存期可选 活期除外的其他存期
			if (madtby == E_MADTBY.T_OR_S || madtby == E_MADTBY.TERMCD) {
...
//		}
		...
		if (CommUtil.isNotNull(custtemp)) {
			throw DpModuleError.DpstProd.BNAS1356();
		}

		KupDppbCustTemp temp = SysUtil.getInstance(KupDppbCustTemp.class);

		temp.setMadtby(madtby);
		temp.setOnlyfg(onlyfg);
		temp.setProdcd(prodcd);
		temp.setSrdpam(srdpam);
		temp.setStepvl(stepvl);
		temp.setCrcycd(tblkup_dppbt.getPdcrcy());

		KupDppbCustTempDao.insert(temp);

		// *录入产品存期控制表

		for (DepttmInfo tmInfo : input.getDepttmInfo()) {
			KupDppbTermTemp termtemp = KupDppbTermTempDao.selectOne_odb1(prodcd, tblkup_dppbt.getPdcrcy(), tmInfo.getDepttm(), false);

			if (CommUtil.isNotNull(termtemp)) {
				throw DpModuleError.DpstProd.BNAS1356();
			}

			KupDppbTermTemp term = SysUtil.getInstance(KupDppbTermTemp.class);

			term.setCrcycd(tblkup_dppbt.getPdcrcy());
			term.setDeptdy(tmInfo.getDeptdy());
			term.setDepttm(tmInfo.getDepttm());
			term.setProdcd(prodcd);

			KupDppbTermTempDao.insert(term);
