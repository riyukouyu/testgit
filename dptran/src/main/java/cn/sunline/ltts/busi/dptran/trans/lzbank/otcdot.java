...
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Otcdot.Input;
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Otcdot.Output;
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Otcdot.Property;
...
public class otcdot {
	...
	public static void dealOtcdot( final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Otcdot.Input input,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Otcdot.Property property,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Otcdot.Output output){
		...
		E_CAPITP capitp =  E_CAPITP.OT204;//资金交易类型-电子账户银联CUPS转出
		//20180521 yanghao
		E_CLACTP clactp = E_CLACTP._34;//清算账户类型
	...
	//		} 	
			 ...
			 */
			//1,内部户贷方记账
			KnpParameter para = KnpParameterDao.selectOne_odb1("InParm.clearbusi","lzbank", "07", "%", true);
	        ...
	        drawDpAcctIn.setOpacna(inacna);
	        drawDpAcctIn.setToacct(inacct);
	        drawDpAcctIn.setCrcycd(tblKnaAcct.getCrcycd());
	        drawDpAcctIn.setBankcd(otbrch);
...
//			entity.setDescrb("银联无卡转出成功");//交易信息    
...
//			entity.setPrepty(prepty);//银联交易类型
			 ...
			 * 9,短信通知
			 */
	//		sendMessage(input,property, output);//转出方为内部户不需发送短信
...
//		//根据卡号查询电子账号
//		IoCaKnaAcdc tblKnaAcdc = CaDao.selKnaAcdcByCard(input.getInacct(), false);
