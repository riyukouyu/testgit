...
public class querspDataProcessor extends
  BatchDataProcessorWithoutDataItem<cn.sunline.ltts.busi.dptran.batchtran.intf.Quersp.Input, cn.sunline.ltts.busi.dptran.batchtran.intf.Quersp.Property> {
	 ...
	 */
	 @Override
	public void process(cn.sunline.ltts.busi.dptran.batchtran.intf.Quersp.Input input, cn.sunline.ltts.busi.dptran.batchtran.intf.Quersp.Property property) {
		 //1、根据文件批次号查询文件流水信息
		 ...
		 FileUtil.readFile(localpath,new FileDataExecutor() {
				...
				public void process(int arg0, String arg1) {
					...
					if(arg0!=1){//文件头不处理
					    ...
					    //String dcn = DcnUtil.findDcnNoByCardNo(ss[2]);
					    String dcn = "000";
					    String filepath = wjplxxb.getLocaph()+wjplxxb.getDownna()+"_"+dcn;
					    if(CommUtil.isNull(totalCache.get(dcn))){
		 ...
		 Iterator<Map.Entry<String, String>> it = fileCache.entrySet().iterator();
		  while (it.hasNext()) {
		   ...
		   String fileContxt = totalCache.get(entry.getKey())+"\n";
