...
public class cupsot {
	 ...
	 * @param output
	 */
	public static void dealCupsot( final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsot.Input input,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsot.Property property,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsot.Output output){
		...
		E_CAPITP capitp = E_CAPITP.OT228;//资金交易类型-银联传统CUPS消费
		// 20180521 yanghao
		...
		if(isflag==E_YES___.NO){//电子账户转内部户
			...
			if(accatp == E_ACCATP.GLOBAL || accatp == E_ACCATP.FINANCE){ //I/II类户
				...
				sbactp = E_SBACTP._12;
				if(input.getPrepty() == E_PREPTY.QX) {
					throw DpModuleError.DpstComm.E9999("III类户不允许做现金交易");
				}
			}
	...
	//		}
			...
			if(input.getPrepty() == E_PREPTY.QX) {
				...
				//客户化状态检查
				cuacst = CapitalTransCheck.ChkAcctstOT(custac);
				//状态字检查
				CapitalTransCheck.ChkAcctstWord(custac);
				//检查对方账号是否绑定账户
//				IoCaKnaCacd cacd = SysUtil.getInstance(IoCaSevGenBindCard.class).selBindByCard(custac, cardno, E_DPACST.NORMAL, false);
				//获取面签属性
				...
				if(CommUtil.isNotNull(cuad.getFacesg()) && cuad.getFacesg() == E_YES___.YES){
					...
					throw DpModuleError.DpstComm.E9999("II类户必须经过面签才能做现金交易");
				}
				
//				if(CommUtil.isNotNull(cacd)){
//					isbind = E_YES___.YES;
//				}
				chkOT = SysUtil.getInstance(AcTranfeChkOT.class);
...
//			entity.setOtbrch(tblKnaAcct.getBrchno());//转出机构         
...
//			entity.setDescrb("银联CUPS转出成功");//交易信息 
...
//			entity.setInacct(inacct);//转入账号         
//			entity.setInacna(inacna);//转入户名         
//			entity.setOtbrch(input.getOtbrch());//转出机构         
//			entity.setInbrch(inbrch);//转入机构         
//			entity.setTrbrch(trbrch);//交易机构         
//			entity.setPrdate(prdate);//前置交易日期     
//			entity.setPrbrmk(prbrmk);//代理机构标识号   
...
//			entity.setPrepty(prepty);//银联交易类型
	 ...
	 * @param input
	 */
	private static void chkParam(cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsot.Input input) {
...
//		}
		...
		if(CommUtil.isNotNull(input.getPrepty())){//银联交易类型
			if(input.getPrepty() == E_PREPTY.ZCZZ) {
				if(CommUtil.isNull(input.getInacct())){//转入账号
					throw DpModuleError.DpstAcct.BNAS0028();
				}
			...
			throw DpModuleError.DpstComm.E9999("银联交易类型不能为空");
		}
...
//                
	 ...
	 * @param output
	 */
	private static void sendMessage(cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsot.Input input, cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsot.Property property,
			cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsot.Output output) {
