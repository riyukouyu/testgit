...
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsin.Input;
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsin.Output;
import cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsin.Property;
...
public class cupsin {
	 ...
	 * @param output
	 */
	public static void dealCupsCancel( final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsin.Input input,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsin.Property property,  final cn.sunline.ltts.busi.dptran.trans.lzbank.intf.Cupsin.Output output){
		...
		String remark = input.getRemark();//摘要   
		String smrycd = BusinessConstants.SUMMARY_ZR;//摘要代码
		E_SBACTP sbactp = E_SBACTP._11;//账户类别-默认为活期结算账户
		E_CAPITP capitp =  E_CAPITP.IN104;//资金交易类型-银联CUPS转电子账户
		//20180521 yanghao
		 ...
		 * 2018/08/16
		 */
		E_ACCTROUTTYPE routeType1 = ApAcctRoutTools.getRouteType(cardno);
...
//			entity.setDescrb("银联CUPS转入成功");//交易信息         
...
//	        if (CommUtil.compare(tblKnaAcct.getOnlnbl(), aa) >= 0){
//	        	
//	        }
            ...
            IoInacInfo glKnaAcct = inQry.InacInfoQuery(cardno);
...
//            }
			...
			acdrIn.setToacct(otacct);
			acdrIn.setToacna(otacna);
			acdrIn.setTranam(tranam);
...
//		cplKubSqrd.setAppsid();//APP推送ID 
		...
		if(E_PREPTY.DJ.equals(input.getPrepty())){
			cplKubSqrd.setPmvl01("DJ");//参数01
		}
...
//		}
		...
		if(CommUtil.isNull(input.getPrepty())){//银联交易类型
		...
		if(E_PREPTY.DJ.equals(input.getPrepty())){
			...
			if(CommUtil.isNull(input.getRemark())){//摘要
				throw DpModuleError.DpstComm.E9999("摘要不能为空！");
			}
		}
