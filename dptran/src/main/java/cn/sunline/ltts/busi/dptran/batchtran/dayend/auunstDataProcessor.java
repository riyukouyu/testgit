...
public class auunstDataProcessor extends
  AbstractBatchDataProcessor<cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Auunst.Input, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Auunst.Property, cn.sunline.ltts.busi.dp.tables.DpFroz.KnbFroz> {
	  /**
		 * 批次数据项处理逻辑。
		 * 
		 * @param job 批次作业ID
		 ...
		 */
		@Override
		public void process(String jobId, int index, cn.sunline.ltts.busi.dp.tables.DpFroz.KnbFroz dataItem, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Auunst.Input input, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Auunst.Property property) {
...
//			CommTools.getBaseRunEnvs().setBusi_org_id(dataItem.getCorpno());
			...
			//外部止付才有紧急止付
			if(dataItem.getFroztp() != E_FROZTP.EXTSTOPAY){
				throw DpModuleError.DpstComm.E9999("外部止付才有紧急止付");
			}
...
//			dpUnStopayIn.setIdno02(dataItem.getIdno01()); //执法人员2证件号码
			dpUnStopayIn.setSfreas("48小时紧急止付的解止"); //解/止付原因
...
//			CommTools.getBaseRunEnvs().setBusi_org_id(oldCorpno);
		 ...
		 */
		@Override
		public BatchDataWalker<cn.sunline.ltts.busi.dp.tables.DpFroz.KnbFroz> getBatchDataWalker(cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Auunst.Input input, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Auunst.Property property) {
			
//			//当前时间往前推48H
//			KnpParameter para = SysUtil.getInstance(KnpParameter.class);
//			
//			para = KnpParameterDao.selectOne_odb1("STOPAY", "EMERGENCY", "%", "%", true);
//			
//			int timval = Integer.valueOf(para.getParm_value1());
//			
//			String trandt = beforeDay(-timval);
			
			Params params = new Params();
			params.add("frozst", E_FROZST.VALID);
			params.add("froztp",  E_FROZTP.EXTSTOPAY);
			params.add("corpno", CommTools.getBaseRunEnvs().getBusi_org_id());
			params.add("trandt", DateTools2.getDateInfo().getSystdt());
			params.add("trantm",  DateTools2.getCurrentLocalTime());
			
			return new CursorBatchDataWalker<KnbFroz>(DpDayEndDao.namedsql_selKnbFrozByStms, params);
