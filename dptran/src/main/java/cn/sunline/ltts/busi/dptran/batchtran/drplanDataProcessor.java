...
public class drplanDataProcessor
		extends
		AbstractBatchDataProcessor<cn.sunline.ltts.busi.dptran.batchtran.intf.Drplan.Input, cn.sunline.ltts.busi.dptran.batchtran.intf.Drplan.Property, cn.sunline.ltts.busi.dp.tables.online.DpDepoBusiMain.KnaDrawPlan> {
	...
	public void process(
			String jobId,
			int index,
			cn.sunline.ltts.busi.dp.tables.online.DpDepoBusiMain.KnaDrawPlan dataItem,
			cn.sunline.ltts.busi.dptran.batchtran.intf.Drplan.Input input,
			cn.sunline.ltts.busi.dptran.batchtran.intf.Drplan.Property property) {
		String acctno_plan = dataItem.getAcctno();//支取计划账号
		...
		if(dpAcct.getAcctst() == E_DPACST.NORMAL){
			...
			KnaAcct inAcct = ActoacDao.selKnaAcctByAcst(acsetp, cplKnaAccs.getCustac(), acctst, true);
			
			// 查询产品账户的可用余额  
			bal = dpAcct.getOnlnbl();
			//检查子账户余额是否能足额支取
			if(CommUtil.compare(bal, dataItem.getPlmony()) >= 0){
				//调用支取记账服务
				drawAcctDp(dpAcct,dataItem.getPlmony());
				//调用存入记账服务
				postAcctDp(inAcct,dataItem.getPlmony());
			}
		}
	}
	/**
	...
	private void postAcctDp(
			KnaAcct acctIn, BigDecimal swpamt) {
	...
	private void drawAcctDp(
			KnaAcctFxacInfo acctOut, BigDecimal swpamt) {
	...
	private KnaAcctFxacInfo setPostIn(IoCaKnaAccs cplKnaAccs){
		...
		if(E_FCFLAG.CURRENT == cplKnaAccs.getFcflag()){
			...
			// 设置产品账户的可用余额  
			dpAcct.setOnlnbl(SysUtil.getInstance(DpAcctSvcType.class)
					.getAcctaAvaBal(cplKnaAccs.getCustac(), cplKnaAccs.getAcctno(),
							cplKnaAccs.getCrcycd(), E_YES___.YES, E_YES___.NO));
			...
			dpAcct.setOnlnbl(knaFxac.getOnlnbl());
		}
		return dpAcct;
	}
	/**
	 ...
	 */
	@Override
	public BatchDataWalker<cn.sunline.ltts.busi.dp.tables.online.DpDepoBusiMain.KnaDrawPlan> getBatchDataWalker(
			cn.sunline.ltts.busi.dptran.batchtran.intf.Drplan.Input input,
			cn.sunline.ltts.busi.dptran.batchtran.intf.Drplan.Property property) {
		log.info("<<===================获取作业数据集合开始======================>>");
		...
		log.info("<<===================获取作业数据集合结束======================>>");
		return new CursorBatchDataWalker<KnaDrawPlan>(
				DpSaveDrawDao.namedsql_selDrawPlansCurrt, param);
	}
