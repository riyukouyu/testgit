...
public class cupstr {
	...
	private static BizLog log = BizLogUtil.getBizLog(cupstr.class);
	/***
	 * 功能：银联CUPS转入电子账户处理
	 * @param input
	 * @param property
	 * @param output
	 */
	public static void dealTransBefore( final cn.sunline.ltts.busi.dptran.trans.client.intf.Cupstr.Input input,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Cupstr.Property property,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Cupstr.Output output){
		...
		KnlIoblCups cups = ActoacDao.selKnlIoblCups(uniseq, false);
		...
		log.debug("<<==交易类型：[%s],交易名称：[%s]==>>");
	}
	
	
	private static void dealTrans(final cn.sunline.ltts.busi.dptran.trans.client.intf.Cupstr.Input input,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Cupstr.Property property,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Cupstr.Output output){

		
		CupsTranfe cupsIN = SysUtil.getInstance(CupsTranfe.class);
		cupsIN.setCrcycd(input.getCrcycd());
		cupsIN.setInacct(input.getInacct());
		cupsIN.setInbrch(input.getInbrch()); //可能为空，在dealCUPSTranfe方法中设置值
		...
		try {
			...
			log.debug("<<======银联转入错误信息：[%s]", e.getLocalizedMessage());
	...
	private static void chkParam(final cn.sunline.ltts.busi.dptran.trans.client.intf.Cupstr.Input input){
