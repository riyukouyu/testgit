...
import cn.sunline.edsp.busi.ds.iobus.type.DsComplex.KubDayFrTotalList;
...
import cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Input;
import cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Property;
...
public class spftmhDataProcessor extends
  AbstractBatchDataProcessor<cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Input, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Property, cn.sunline.edsp.busi.ds.iobus.type.DsComplex.KubDayFrTotalList> {
	
	
		@Override
		public void beforeTranProcess(String taskId, Input input, Property property) {
			//获取配置的几号跑
			KnpParameter datePara = KnpParameterDao.selectOne_odb1("Dp.Param", "spftmh", "spftDate", "date", true);
			String dateAdd = DateTools.dateAdd("D", CommTools.getBaseRunEnvs().getTrxn_date(), - (Integer.parseInt(datePara.getParm_value1()) + 1));
			String sTranMonth = dateAdd.substring(0,6);			
			
			property.setSpmhdt(datePara.getParm_value1());
			property.setTranmh(sTranMonth);
		}
		/**
		 * 批次数据项处理逻辑。
		 * 
		 ...
		 */
		@Override
		public void process(String jobId, int index, cn.sunline.edsp.busi.ds.iobus.type.DsComplex.KubDayFrTotalList dataItem, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Input input, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Property property) {
			...
			CommTools.getBaseRunEnvs().setMain_trxn_seq(mntrsq);
			...
			if(CommUtil.compare(BigDecimal.ZERO, prftam) != 0) {
				...
				if(CommUtil.compare(BigDecimal.ZERO, prftam) < 0) {
					...
					cplSaveAcctIn.setRemark("月结分润计提存入");
					cplSaveAcctIn.setSmrycd("YJFRJT");
			...
			// 记录处理
			DpAcctDao.updKubMothFrDealst(knaSbad.getSbrand(), knaSbad.getBradna(), 
					E_DEALST.SUCCESS, dataItem.getAgntid(), property.getTranmh(), 
		...
		public void jobExceptionProcess(String taskId, Input input, Property property, String jobId,
				KubDayFrTotalList dataItem, Throwable t) {
			DpAcctDao.updKubMothFrDealst(null, null, E_DEALST.FAIL, dataItem.getAgntid(), 
					property.getTranmh(),CommTools.getBaseRunEnvs().getTrxn_code(), null);
		}	
		 ...
		 */
		@Override
		public BatchDataWalker<cn.sunline.edsp.busi.ds.iobus.type.DsComplex.KubDayFrTotalList> getBatchDataWalker(cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Input input, cn.sunline.ltts.busi.dptran.batchtran.dayend.intf.Spftmh.Property property) {
			
			// 判断是否是当前日期
			String trandt = CommTools.getBaseRunEnvs().getTrxn_date();
			if(CommUtil.equals(property.getSpmhdt(), trandt.substring(6,8))) {
				Params params = new Params();
				params.add("sTranMonth", property.getTranmh()+"%");
				return new CursorBatchDataWalker<cn.sunline.edsp.busi.ds.iobus.type.DsComplex.KubDayFrTotalList>(DpAcctDao.namedsql_selAllAgntMothFrTotal, params);
			}else {
				return null;
			}
		}
