...
public static void chkAcpaytInfo( final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Input input,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Property property,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Output output){
		...
		if (CommUtil.isNull(input.getMebrch())) {
			//throw DpModuleError.DpstAcct.E9999("商户机构不能为空");
		}
//		if (CommUtil.isNull(input.getMecard())) {
//			throw DpModuleError.DpstAcct.E9999("商户账号/卡号不能为空");
//		}
		if (CommUtil.isNull(input.getMercnm())) {
			//throw DpModuleError.DpstAcct.E9999("商户号不能为空");
		}
		if (CommUtil.isNull(input.getMercsq())) {
			//throw DpModuleError.DpstAcct.E9999("商户交易流水不能为空");
		}
		if (CommUtil.isNull(input.getMercdt())) {
			//throw DpModuleError.DpstAcct.E9999("商户交易日期不能为空");
		}
		if (CommUtil.isNull(input.getMerctm())) {
			//throw DpModuleError.DpstAcct.E9999("商户交易时间不能为空");
		}
...
//		chkqtn(isckqt, custtp, limttp, risklv, authtp, sbactp, pytltp, rebktp, custlv, aclmfg, custid);
		...
		property.setAccttp(accatp);
		property.setAcsetp2(tblKnaAcct.getAcsetp());
		// 取得收款方法人代码
		String incorp = null;
		try{
			incorp = SysUtil.getInstance(IoSrvPbBranch.class).getBranch(input.getMebrch()).getCorpno();
		}
		catch (Exception e) {
			// 不处理
		}
...
//		property.setServtp(String.ME);// 报文头交易渠道有问题，取出为交易码
		...
		property.setSpettp(E_SPETTP.SPEND);// 电子账户
	}

	public static void chkTransAfter(final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Input input, final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Property property, final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Output output) {
		...
		ioCheckBalance.checkBalance(trandt, transq,E_CLACTP._01);//借：电子账户C:柜面核心往来
	}
	 ...
	 * @param output
	 * @author liaojincai
	 * @date 2016年8月1日 下午4:07:11 
	 * @version V2.3.0
	 */
	public static void prcyInacRegister( final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Input input,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Property property,  final cn.sunline.ltts.busi.dptran.trans.client.intf.Acpayt.Output output){
		
		E_INSPFG invofg = property.getInvofg();// 是否涉案
		...
		if (E_INSPFG.INVO == invofg) {
			...
			DaoUtil.executeInNewTransation(new RunnableWithReturn<Void>() {
				...
				public Void execute() {
					...
					cplKnbTrin.setIncard(input.getMecard());// 转入账号
					cplKnbTrin.setInbrch(input.getMebrch());// 转入机构
