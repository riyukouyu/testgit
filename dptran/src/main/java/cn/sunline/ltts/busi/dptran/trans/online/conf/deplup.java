...
public class deplup {
	 ...
	 * 
	 * @Title: depupt
	 * @Description: 修改存款存入计划控制部件信息
	 * @param input
	 * @author zhangjunlei
	 * @date 2016年7月13日 下午10:10:45
	 * @version V2.3.0
	 */
	public static void depupt(
			final cn.sunline.ltts.busi.dptran.trans.online.conf.intf.Deplup.Input input) {
		...
		KupDppbPoplTemp tblKup = SysUtil.getInstance(KupDppbPoplTemp.class);// 存入计划控制表

		// 判断存入存入计划控制临时表有没有已存在的产品
		tblKup = KupDppbPoplTempDao.selectOne_odb1(prodcd, crcycd, false);
		if (CommUtil.isNull(tblKup)) {
			throw DpModuleError.DpstComm.BNAS2003();
		}
		...
		if (tblDppb_part_temp.getPartfg() == E_YES___.NO) {
			...
			KupDppbPoplTempDao.updateOne_odb1(tblKup);// 修改存款存入计划控制部件信息
		}
	}
